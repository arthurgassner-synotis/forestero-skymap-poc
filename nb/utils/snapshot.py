...
@dataclass
class Snapshot:
    rgb_re_nir_swir_scl: np.ndarray  # RGB RE NIR SWIR
    ...
    def red(self) -> np.ndarray:
        raw_red = self.rgb_re_nir_swir_scl[:, :, 0]
    ...
    def green(self) -> np.ndarray:
        raw_green = self.rgb_re_nir_swir_scl[:, :, 1]
    ...
    def blue(self) -> np.ndarray:
        raw_blue = self.rgb_re_nir_swir_scl[:, :, 2]
    ...
    def red_edge(self) -> np.ndarray:
        raw_red_edge = self.rgb_re_nir_swir_scl[:, :, 3]
    ...
    def nir(self) -> np.ndarray:
        raw_nir = self.rgb_re_nir_swir_scl[:, :, 4]
    ...
    def swir(self) -> np.ndarray:
        raw_swir = self.rgb_re_nir_swir_scl[:, :, 5]
    ...
    @property
    def scl(self) -> np.ndarray:
        return self.rgb_re_nir_swir_scl[:, :, 6]
    ...
    @staticmethod
    def load_from_s2item(s2item: S2Item, bbox_wgs84: tuple[float, float, float, float], padding_m: float) -> "Snapshot":
        ...
        band_names = ["red", "green", "blue", "rededge1", "nir", "swir22", "scl"]
        results = {}

        with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
            future_to_band = {
                executor.submit(Snapshot._load_from_tif, p / f"{p.name}_{band}.tif", lonlats_wgs84, padding_m): band for band in band_names
            }
        ...
        scl_zoom_y = red.shape[0] / scl.shape[0]
        scl_zoom_x = red.shape[1] / scl.shape[1]
        scl = zoom(scl, zoom=(scl_zoom_y, scl_zoom_x), order=1)

        rgb_re_nir_swir_scl = np.dstack((red, green, blue, red_edge, nir, swir, scl)).astype("float32")
        ...
        return Snapshot(stac_item=s2item.stac_item, rgb_re_nir_swir_scl=rgb_re_nir_swir_scl, bounds=bounds, crs=crs)

    @property
    def features(self) -> np.ndarray:
        red = np.median(self.red)
        green = np.median(self.green)
        blue = np.median(self.blue)
        ndvi = np.median(self.ndvi)
        tci = np.median(self.tci)
        nir = np.median(self.nir)
        swir = np.median(self.swir)
        gndvi = np.median(self.gndvi)
        scl = np.max(self.scl)

        return np.array([ndvi, tci, nir, swir, gndvi, scl, red, green, blue])
